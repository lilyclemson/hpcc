...
tourDescription = "This profile provides a configurable HPCCSystems testbed one master and customizable thorslaves (multiples of four)using Utah m510."
...
for i in range(params.workerCount + 1):
    ...
    node.addService(rspec.Execute(shell="/bin/sh",
                                  command="sudo apt-get -y install libarchive13 ibatlas3-base ibboost-regex1.54.0 libicu52 libtbb2 libxslt1.1 expect zip"))
#    node.addService(rspec.Execute(shell="/bin/sh",
#                                  command="sudo wget http://cdn.hpccsystems.com/releases/CE-Candidate-5.2.2/bin/platform/hpccsystems-platform-community_5.2.2-1trusty_amd64.deb"))
    node.addService(rspec.Execute(shell="/bin/sh",
                                  command="sudo wget http://wpc.423a.rhocdn.net/00423A/releases/CE-Candidate-6.4.2/bin/platform/hpccsystems-platform-community_6.4.2-1trusty_amd64.deb"))
                                  
    node.addService(rspec.Execute(shell="/bin/sh",
                                  command="sudo dpkg -i hpccsystems-platform-community_6.4.2-1trusty_amd64.deb"))
    node.addService(rspec.Execute(shell="/bin/sh",
